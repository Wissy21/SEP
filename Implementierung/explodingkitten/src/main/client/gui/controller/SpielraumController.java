...
import main.client.gui.controller.Spiel.KarteController;
import main.client.gui.controller.Spiel.OpponentController;
...
public class SpielraumController implements IRaumObserver {
     ...
     */
    public void setName(String name, String raumname, String ip) {
        this.name = name;
        ...
        try {
            ...
            for (Spieler s : sb.getSpieler()) {
                if (!s.getNickname().equals(name)) {
                    FXMLLoader loader = new FXMLLoader(SpielenClient.class.getResource("gui/vue/Spiel/Opponent.fxml"));
                    ...
                    for (Karte k : s.getHandkarte()) {
                        FXMLLoader loader = new FXMLLoader(SpielenClient.class.getResource("gui/vue/Spiel/Karte.fxml"));
                        Parent neuekarte = loader.load();
                        KarteController kc = loader.getController();
                        kc.setKarte(k, name, raumname, serverIp);
    ...
    public void zeigeRegeln() {
        FXMLLoader loader = new FXMLLoader(SpielenClient.class.getResource("gui/vue/Spiel/Regeln.fxml"));
        Parent root = null;
        try {
            root = loader.load();
        } catch (IOException e) {
        ...
        if (name.equals(spielername)) {
            switch (message) {
                ...
                case "AbgelegtDu":
                    ...
                    Platform.runLater(() -> ablage.setImage(new Image(SpielenClient.class.getResource("gui/images/Karten/" + k.getEffekt() + ".png").toString(), 0, 150, true, false)));
                    break;
